...
+ (void)load
{
    [CoreDataCenter shareInstance];
}

+ (instancetype)shareInstance
{
    static CoreDataCenter *center = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        center = [[CoreDataCenter alloc] init];
    });
    return center;
}
...
- (CFling *)newCFling {
    ...
    NSEntityDescription *flingDes = [NSEntityDescription entityForName:@"CFling" inManagedObjectContext:context];
    CFling *fling = [[CFling alloc] initWithEntity:flingDes insertIntoManagedObjectContext:context];
...
- (CMessage *)newCMessage {
    ...
    CMessage *message = [[CMessage alloc] initWithEntity:messageDes insertIntoManagedObjectContext:context];
    return message;
}

- (NSFetchedResultsController *)fetchedResultsControllerAllFling {
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CFling" inManagedObjectContext:self.managedObjectContext];
    [fetchRequest setEntity:entity];
    [fetchRequest setFetchBatchSize:20];
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"time" ascending:NO];
    NSArray *sortDescriptors = @[sortDescriptor];
    [fetchRequest setSortDescriptors:sortDescriptors];
    NSFetchedResultsController *aFetchedResultsController = [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:self.managedObjectContext sectionNameKeyPath:nil cacheName:@"Fling"];
    NSError *error = nil;
    if (![aFetchedResultsController performFetch:&error]) {
        NSLog(@"Unresolved error %@, %@", error, [error userInfo]);
        abort();
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.dongyin.Fling" in the application's documents directory.
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"Fling" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"Fling.sqlite"];
    NSError *error = nil;
